...
import { INITIAL_STOCK, INITIAL_DISTRIBUTIONS } from '@/lib/store'
import { Plus, History, ArrowDownToLine, ArrowUpToLine, Package, User } from 'lucide-react'
...
export default function InventoryPage() {
  ...
  const renderFacilityInventory = (facilityName: string) => {
    ...
    const filteredDistributions = distributions.filter(d => d.facility === facilityName);
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-primary/10 border-primary/20">
            <CardHeader className="p-4">
              <CardTitle className="text-sm flex items-center gap-2">
                ...
                Total Received (from Warehouse)
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-0">
              <p className="text-3xl font-black text-primary">{facilityReceived.toLocaleString()}</p>
              <p className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Node Capacity</p>
            </CardContent>
          </Card>
          <Card className="bg-accent/10 border-accent/20">
            <CardHeader className="p-4">
              <CardTitle className="text-sm flex items-center gap-2">
                ...
                Total Dispensed (to Peers)
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-0">
              <p className="text-3xl font-black text-accent">{facilityDispensed.toLocaleString()}</p>
              <p className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Community Outreach</p>
            </CardContent>
          </Card>
          <Card className="bg-green-500/10 border-green-500/20">
            ...
            <CardContent className="p-4 pt-0">
              ...
              <p className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Facility Balance</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Peer Distribution Ledger</CardTitle>
            <CardDescription>Real-time log of stock dispensed to Peers across Wards</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Peer UIN</TableHead>
                ...
                {filteredDistributions.length > 0 ? (
                  filteredDistributions.map((dist) => (
                    <TableRow key={dist.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className="font-bold">{dist.ward}</Badge>
                      </TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                      No peer distributions logged for this facility yet.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Warehouse &rarr; Facility Nodes &rarr; Peer Distribution</p>
        </div>
      ...
      <Tabs defaultValue="matapi" className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-8 h-12">
          <TabsTrigger value="matapi" className="text-base font-bold">Matapi Youth Hub (Wards 3 & 4)</TabsTrigger>
          <TabsTrigger value="edith" className="text-base font-bold">Edith Opperman (Wards 11 & 12)</TabsTrigger>
        </TabsList>
